...
** Fix
- Add city names back to Publisher: field
- Fix one Authors to be "and First Last" or
- Fix multiple Authors to be "First Last, First Last, and First Last"
- Fix up the generation of the bib table. I.e. is it usable?
  - For Id, prefix with '{', end with '} '
  - If Author and no Authors, append ". " to Author
  - If Author and Authors, append ", " to Author
  - If Authors is not empty, end with '. '
  - Put '. ' at end of Date
  - If Title ends with ", Change " to '."', else end it with '. '
  - If Publisher, put '. ' at end of Publisher
  - If AltLink, then only copy the first one and prefix it with "; "
- Fix the formatting, see etc/bib-template.xml
  - [Id][Author][Authors][Date][Title][Publisher][Link][AltLink]
** Add tags [TBD]
- Subtitle:
  - get this from old Title tag
- Edition: 2nd, 3rd, Nth
  - get this from old Publisher tag
  - empty, if 1st, or First
